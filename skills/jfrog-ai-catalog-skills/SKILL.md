...
- **Resolve the server once (`<SID>`), then reuse it for the whole session.**
  ...
  never shell out to `python3` for this. Pass `--server-id <SID>` after the
...
- **Resolve the project (`<PROJECT>`) only when needed.** It is required for
  `--list-skills` (browse/name-search) and `--provision-skills-repository`
  (auto-creating a publish repo when the user names none). Resolve it from
  `JF_PROJECT`, else ask the user. Never assume `default`, never invent one.
  `--list-skill-versions`, install, update, remove, and publishing to an explicit
  `--repo` are keyed by skill **name** and/or **repo**, not a project.
- **Confirm before mutating.** Install and list are read-mostly. **Remove**,
  registry **delete**, and **publish** mutate state. Confirm with the user
  first, prefer a read to check current state, and never invent preparatory
  mutations (creating repos, deleting versions) the user did not ask for.
- **On any `jf` error, stop, never switch servers.** If a call returns
  401/403/404, a network error, or a timeout, stop with no further `jf` calls and
  report it. Do not retry against another configured server unless the user
...
- **Use the response templates verbatim**: where a reference file gives a "reply
  ...
  with the same wording every time and no extra preamble or commentary. This keeps
  replies consistent across runs.
