...
**Installing a named skill needs no project.** Never ask the user for one here.
...
npx --yes --registry <REGISTRY_URL> @jfrog/agent-guard \
  --list-skill-versions --skill "<slug>" [--server "<SID>"] --format json
...
This is a security control. **Do not silently bypass it.** Stop, tell the user
the skill has no evidence, and ask whether to proceed without it. Only if the
user explicitly agrees, re-run with `JFROG_SKILLS_DISABLE_QUIET_FAILURE=true`.
Never set that flag on your own.
...
- **`SCAN_IN_PROGRESS`** → Xray is still scanning the archive. The download is
  ...
  **this exact template** (no extra prose):
...
- **Blocked by a policy** (a blocked/violating status, with the offending
  ...
  Stop. Do not retry. Reply using **this exact template** (fill the
  placeholders, no extra prose):
  ...
  > the policy.
- **Any other status / non-200** → treat as an operational failure (auth,
...
On success, reply using **this exact template** (no extra prose):
