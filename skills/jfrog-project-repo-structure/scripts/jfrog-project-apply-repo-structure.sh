#!/usr/bin/env bash
# jfrog-project-apply-repo-structure.sh — Apply Phase 3+4 sections of a JFrog
# Project template idempotently.
#
# Reads a JFrog project template (see assets/project-templates/schema.json
# in the base jfrog skill) and reconciles its `stages`, `repositories`,
# `external_stage_rbac`, and `sharing` sections with the JFrog Platform
# using GET-before-PUT/POST. Phase 1+2 sections (project, admins, members,
# oidc) are ignored here — they belong to jfrog-project-create-from-template.sh.
#
# All API calls go through `jf api` against the resolved server. Caller must
# have Project-Admin (on the target project) or Platform-Admin scope. Network
# permission is required (run via the Shell tool with
# required_permissions: ["full_network"]).
#
# Usage:
#   jfrog-project-apply-repo-structure.sh <template.json> [--server-id <id>] [--dry-run] [--strict-naming]
#
# Arguments:
...
if [[ -z "$TEMPLATE_PATH" ]]; then
  echo "Usage: $0 <template.json> [--server-id <id>] [--dry-run] [--strict-naming]" >&2
  exit 1
fi
...
WORKDIR=$(mktemp -d -t jfrog-project-apply-3-4.XXXXXX)
...
# ---------------------------------------------------------------------------
# Section 1: Verify project exists and caller has scope
# ---------------------------------------------------------------------------
