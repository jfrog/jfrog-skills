...
| Reference architecture | Sizing or use-case answers cite numbers not on the official site, or `small` recommended for production | Agent used training data instead of live ref-arch fetch | Use `jfrog-reference-architecture` skill: `WebFetch` `llms-full.txt` first; cite `URL:` from fetched sections; see `../jfrog-reference-architecture/references/doc-access.md` | SaaS paths use `jfrog-saas`; HA needs `cluster-file-system` or object storage, not `file-system` per ref arch |
