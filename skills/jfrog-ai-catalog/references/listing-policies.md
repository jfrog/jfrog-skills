...
download. This is **Unified Policy** governance (the AI Catalog's own policy
...
`default`, never invent one), then call Unified Policy **directly** — this is
not an Artifactory or Xray product, so it isn't in the base skill's `jf api`
product-prefix table, but the same authenticated `jf api` mechanism reaches it
on the same JPD:
...
Policies affecting skills in project `<PROJECT>`:
...
> No skill governance policies apply to project `<PROJECT>`.

**This single line also covers the case where the caller isn't authorized to
see the project's policies** — Unified Policy returns the identical empty
shape for "nothing here" and "you can't see this," and there is no non-admin
way to tell them apart (the same limitation already noted for project-key
...
- **Never call the `/ui/api/v1/...` gateway path.** That's the browser
  session's door into Unified Policy and rejects a bearer token outright.
  The path above (`/unifiedpolicy/api/v1/...`, no `/ui/api/v1` prefix) is the
  one a CLI/token caller uses — same door the waiver flow already goes
  through.
